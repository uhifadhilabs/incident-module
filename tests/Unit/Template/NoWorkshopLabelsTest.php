 ...
 *
 * Every widget in the design files carries an identifier — IN·07, IN·K1, PL·05,
 * TM·02 — printed in the card's tab. That is the workspace's own referencing
 ...
 * is not product. A warden reading a case file has no decisions table, cannot
 * follow the reference, and reads "IN·04Timeline" as a typo.
 *
 ...
 * instances somebody happened to notice: no template may emit a widget
 * identifier, in any of the forms Twig can write one, and the chip's own class
 * is refused outright — including the interpolated spellings this module
 * invented, where the step number of a workflow place is written into the
 * label.
 *
...
final class NoWorkshopLabelsTest extends TestCase
{
    ...
    private const array FORBIDDEN = [
        ...
        'literal middot' => '/\b(?:TM|PM|DP|MB|AU|FL|PL|IN)\x{00B7}/u',
        'named entity' => '/\b(?:TM|PM|DP|MB|AU|FL|PL|IN)&middot;/',
        'numeric entity' => '/\b(?:TM|PM|DP|MB|AU|FL|PL|IN)&#(?:183|xB7);/i',
    ];
