 ...
 * as {@see \Uhifadhi\Incident\Module\IncidentDepartmentKpiProvider} already
 * states for the KPI plates. All a department contributes to a figure is WHICH
 * GROUND it reads, and that is the whole of what this resolves.
 *
 * THE CASES, AND THEY ARE ONE RULE. The slice is the intersection of the
 ...
 * department confined to somewhere else is not a row with no figures, it is
 * not a row at all, which is the same distinction the matrix draws between a
 * dash and an absent row.
 *
 * It is a model, not an entity: made for one render and thrown away.
 */
final readonly class IncidentTopicSlice
{
